...
// ── YouTube video stats (used in trailer info bar) ───────────
...
// ── UPDATED download status — honest reporting ───────────────
// Replace the existing /api/download/status with this:
app.get('/api/download/status', (req, res) => {
    const ytDlpReady = !!YT_DLP_PATH;
    const ytdlReady  = !!ytdl;
    const wtReady    = !!WebTorrent;
    ...
    res.json({
        ...
        data: {
            ...
            activeJobs:   wtClient ? wtClient.torrents.length : 0,
            features: {
                trailerDownload: ytDlpReady || ytdlReady,
                movieDownload:   wtReady,
                tvDownload:      wtReady,
            },
        },
    });
...
// ── UPDATED trailer download — better yt-dlp args ────────────
// Replace the existing /api/download/trailer/:videoId with this:
app.get('/api/download/trailer/:videoId', async (req, res) => {
    const { videoId } = req.params;
    const { title = 'trailer' } = req.query;
    ...
    // Try yt-dlp first
    if (YT_DLP_PATH) {
        console.log(`[yt-dlp] Downloading trailer: ${videoId}`);
        res.setHeader('Content-Type', 'video/mp4');
        res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
        res.setHeader('Transfer-Encoding', 'chunked');
        ...
        proc.stdout.on('data', chunk => {
            ...
            try { res.write(chunk); } catch (_) { proc.kill('SIGTERM'); }
        });
        ...
        proc.on('error', err => {
            ...
            if (!res.headersSent) {
                ...
                res.status(500).json({ success: false, error: 'yt-dlp failed: ' + err.message });
            }
        });
    ...
    if (ytdl) {
        try {
            ...
            if (!format) return res.status(404).json({ success: false, error: 'No suitable format found' });
    ...
    // No engine — tell client to use external tool
    res.status(503).json({
        success: false,
        error: 'No download engine available on this server. Please use Y2Mate or SaveFrom.',
    });
});
