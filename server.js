...
// Named caches with appropriate TTLs
const cache = {
    ...
    for (const query of searchQueries) {
        ...
        try {
            ...
            const scoredResults = data.items.map(item => {
                ...
                studios.forEach(s  => { if (channelTitle.includes(s))  score += 20; });
    ...
    try {
        ...
        if (ytResult.success) {
            ...
            return {
                ...
                type:         'Trailer',
                name:         ytResult.title,
                official:     ytResult.score > 50,
                source:       'youtube',
                channelTitle: ytResult.channelTitle,
                publishedAt:  ytResult.publishedAt
            };
        }
...
app.get('/api/embed/movie/:tmdbId',
    cacheMiddleware(cache.embed, req => `embed:movie:tmdb:${req.params.tmdbId}`),
    async (req, res) => {
...
app.get('/api/embed/movie/imdb/:imdbId',
    cacheMiddleware(cache.embed, req => `embed:movie:imdb:${req.params.imdbId}`),
    async (req, res) => {
...
app.get('/api/embed/tv/:tmdbId/:season/:episode',
    cacheMiddleware(cache.embed, req => `embed:tv:tmdb:${req.params.tmdbId}:${req.params.season}:${req.params.episode}`),
    async (req, res) => {
...
app.get('/api/embed/tv/imdb/:imdbId/:season/:episode',
    cacheMiddleware(cache.embed, req => `embed:tv:imdb:${req.params.imdbId}:${req.params.season}:${req.params.episode}`),
    async (req, res) => {
...
// DETAILS (with everything appended)
        ...
        try {
            ...
            res.json({
                ...
                data: {
                    ...
                    title:         'Unavailable',
                    name:          'Unavailable',
                    overview:      "Sorry, we couldn't load details.",
                    poster_path:   null,
                    backdrop_path: null,
                    vote_average:  0,
                    genres:        [],
...
async function computeSimilarityScore(sourceItem, candidateItem) {
    ...
    const yearScore     = Math.max(0, 1 - (Math.abs(sourceYear - candidateYear) / 20));

    const sourceRating    = parseFloat(sourceItem.vote_average)    || 5;
    const candidateRating = parseFloat(candidateItem.vote_average) || 5;
    const ratingScore     = Math.max(0, 1 - (Math.abs(sourceRating - candidateRating) / 5));
...
async function getEnhancedRecommendations(id, type) {
    try {
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`\n✅ Trailer: GET /api/trailer/:type/:id  (TMDB → YouTube fallback)`);
    console.log(`✅ Health:  GET /api/health`);
    console.log(`\n🎬 All TMDB + YouTube endpoints active.\n`);
});
