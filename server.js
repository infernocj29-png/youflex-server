...
async function getYouTubeTrailer(title, year, type) {
    try {
        // Search for official trailer only (not shorts, not behind the scenes)
        ...
        const res = await axios.get('https://www.googleapis.com/youtube/v3/search', {
            params: {
                ...
                videoDuration: 'medium', // medium = 4-20 min, filters out shorts
                maxResults: 10,
        ...
        // Filter for real trailers — exclude shorts, behind the scenes, clips
        ...
        const trailers = items.filter(item => {
            ...
            const isExcluded = excluded.some(ex => t.includes(ex));
            const isTrailer  = t.includes('trailer') || t.includes('teaser');
            return isTrailer && !isExcluded;
        });

        // Pick official trailer first, then any trailer
...
try {
    ...
    console.warn('⚠️  ytdl-core not found');
}
...
try {
    ...
    console.warn('⚠️  WebTorrent not found');
}
...
app.get('/api/discover/:type', async (req, res) => {
    try {
        const { type } = req.params;
        const data = await tmdbFetch(`/discover/${type}`, req.query);
        res.json({ success: true, data });
    } catch (err) {
        res.status(500).json({ success: false, error: err.message });
    }
...
app.get('/api/trailer/:type/:id', async (req, res) => {
    try {
        ...
        // Filter for real trailers only — exclude shorts, BTS, clips
        ...
        if (tmdbTrailer) {
            return res.json({
                success: true,
                data: {
                    videoId: tmdbTrailer.key,
                    title: tmdbTrailer.name,
                    source: 'tmdb',
                },
            });
        }
...
app.get('/api/embed/tv/:tmdbId/:season/:episode', (req, res) => {
    ...
    const sources = EMBED_SOURCES.map(s => ({
        name: s.name,
        url:  s.tv(tmdbId, season, episode),
    }));
    res.json({ success: true, data: { sources } });
});
...
async function searchEZTV(tmdbId, season, episode) {
    try {
        ...
        const epMatch = torrents.filter(t => {
            ...
            return tn.includes(`s${s}e${e}`) || tn.includes(`${season}x${String(episode).padStart(2,'0')}`);
        });
        ...
        console.log(`✅ Found: ${torrent.title} — Seeds: ${torrent.seeds}`);
...
function streamTorrent(magnetUrl, filename, res, req) {
    return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
            reject(new Error('Torrent timeout — no peers found in 60s'));
        }, 60000);
...
app.get('/api/download/trailer/:videoId', async (req, res) => {
    ...
    if (YT_DLP_PATH) {
        ...
        proc.on('close', code => {
            if (code !== 0) console.error(`❌ yt-dlp trailer failed: code ${code}`);
    ...
    try {
        ...
        if (!torrentInfo?.magnet) {
            return res.status(404).json({
                success: false,
                error: `No torrent found for this ${type}. It may not be available yet.`,
            });
        }
