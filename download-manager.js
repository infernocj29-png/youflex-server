...
.dl-quality-btn .ql-size{font-size:0.7rem;color:#8e9bb0}
...
async function checkDownloadCapability() {
  ...
  try {
    ...
    _dlCapable = data?.data || { ytDlp: false, ytdlCore: false };
  } catch (_) {
    _dlCapable = { ytDlp: false, ytdlCore: false };
  }
  return _dlCapable;
}
...
async function showQualityPicker({ tmdbId, type, title, season, episode }) {
  ...
  // Try to fetch available qualities from server
  let qualities = null;
  try {
    const qs = new URLSearchParams({ season: season || 1, episode: episode || 1 });
    const res = await fetch(`${API_BASE_DL}/download/info/${type}/${tmdbId}?${qs}`);
    const data = await res.json();
    if (data.success && data.data?.qualities?.length) {
  ...
  // Fallback quality options if server info unavailable
  if (!qualities) {
    qualities = [
  ...
  // Always ensure "Best Available" option exists
  if (!qualities.find(q => q.value === 'best')) {
    qualities.push({ label: 'Best Available', value: 'best' });
  }

  list.innerHTML = qualities.map((q, i) => {
    const sizeLabel = q.filesize ? ` (~${(q.filesize / 1024 / 1024).toFixed(0)} MB)` : '';
    const badge = i === 0 ? '<span class="ql-badge">Best</span>' : '';
    return `
      <button class="dl-quality-btn ${i === 0 ? 'active' : ''}" data-quality="${q.value}">
        <span>${q.label}${sizeLabel}</span>
        ${badge}
      </button>`;
  }).join('');
  ...
  list.querySelectorAll('.dl-quality-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      showDownloadToast(`Starting download${quality !== 'best' ? ` (${quality}p)` : ''}…`);
...
// ── TRAILER DOWNLOAD (no quality picker — ytdl-core handles it) ──
